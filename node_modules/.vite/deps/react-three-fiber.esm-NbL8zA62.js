...
import { Camera, Clock, Layers, OrthographicCamera, PerspectiveCamera, Raycaster, Scene, Vector2, Vector3, WebGLRenderer, t as three_module_exports } from "./three.js";
...
//#endregion
//#region node_modules/@react-three/fiber/node_modules/zustand/esm/index.js
...
var useIsomorphicLayoutEffect$2 = typeof window === "undefined" || !window.navigator || /ServerSideRendering|^Deno\//.test(window.navigator.userAgent) ? import_react.useEffect : import_react.useLayoutEffect;
	...
	module.exports = function $$$reconciler($$$hostConfig) {
		...
		*/
		...
		function pushInterleavedQueue(queue) {
			if (interleavedQueues === null) interleavedQueues = [queue];
				...
				case HostRoot:
					...
					switch (fiber.tag) {
						...
						case HostText:
							...
							break;
						case SuspenseComponent: didNotFindHydratableSuspenseInstanceWithinContainer(parentContainer);
					}
					break;
				case HostComponent:
					...
					switch (fiber.tag) {
						...
						case HostText:
							...
							break;
						case SuspenseComponent: didNotFindHydratableSuspenseInstance(parentType, parentProps, parentInstance);
					}
					break;
				case SuspenseComponent:
					...
					if (_parentInstance !== null) switch (fiber.tag) {
						...
						case HostText:
							...
							break;
						case SuspenseComponent: didNotFindHydratableSuspenseInstanceWithinSuspenseInstance(_parentInstance);
					}
					break;
				default: return;
			}
		...
		function prepareToHydrateHostInstance(fiber, rootContainerInstance, hostContext) {
			if (!supportsHydration) throw new Error("Expected prepareToHydrateHostInstance() to never be called. This error is likely caused by a bug in React. Please file an issue.");
			var instance = fiber.stateNode;
		...
		function prepareToHydrateHostTextInstance(fiber) {
			if (!supportsHydration) throw new Error("Expected prepareToHydrateHostTextInstance() to never be called. This error is likely caused by a bug in React. Please file an issue.");
			var textInstance = fiber.stateNode;
			var textContent = fiber.memoizedProps;
			var shouldUpdate = hydrateTextInstance(textInstance, textContent, fiber, !didSuspend);
			if (shouldUpdate) {
		...
		function prepareToHydrateHostSuspenseInstance(fiber) {
			if (!supportsHydration) throw new Error("Expected prepareToHydrateHostSuspenseInstance() to never be called. This error is likely caused by a bug in React. Please file an issue.");
		...
		function skipPastDehydratedSuspenseInstance(fiber) {
			if (!supportsHydration) throw new Error("Expected skipPastDehydratedSuspenseInstance() to never be called. This error is likely caused by a bug in React. Please file an issue.");
			var suspenseState = fiber.memoizedState;
			var suspenseInstance = suspenseState !== null ? suspenseState.dehydrated : null;
			if (!suspenseInstance) throw new Error("Expected to have a hydrated suspense instance. This error is likely caused by a bug in React. Please file an issue.");
		...
		*/
		...
		function dispatchReducerAction(fiber, queue, action) {
			...
			if (isRenderPhaseUpdate(fiber)) enqueueRenderPhaseUpdate(queue, update);
				...
				var root = scheduleUpdateOnFiber(fiber, lane, requestEventTime());
		...
		*/
			...
			if ((suspenseBoundary.mode & ConcurrentMode) === NoMode) {
				if (suspenseBoundary === returnFiber) suspenseBoundary.flags |= ShouldCapture;
					...
					if (supportsPersistence && enablePersistentOffscreenHostContainer) {
						if (suspenseBoundary.alternate === null) {
							var offscreenContainer = suspenseBoundary.child.child;
		...
		function hadNoMutationsEffects(current, completedWork) {
			if (current !== null && current.child === completedWork.child) return true;
			if ((completedWork.flags & ChildDeletion) !== NoFlags) return false;
			var child = completedWork.child;
			while (child !== null) {
				if ((child.flags & MutationMask) !== NoFlags || (child.subtreeFlags & MutationMask) !== NoFlags) return false;
				child = child.sibling;
			}
			return true;
		}
			...
			appendAllChildren = function(parent, workInProgress, needsVisibilityToggle, isHidden) {
				...
				while (node !== null) {
					if (node.tag === HostComponent) {
						...
						var child = node.child;
						if (child !== null) child.return = node;
		...
		function completeWork(current, workInProgress, renderLanes) {
			...
			switch (workInProgress.tag) {
				...
				case SuspenseComponent:
					...
					else prevDidTimeout = current.memoizedState !== null;
					...
					if ((workInProgress.mode & ProfileMode) !== NoMode) {
						if (nextDidTimeout) {
							...
							if (_primaryChildFragment2 !== null) workInProgress.treeBaseDuration -= _primaryChildFragment2.treeBaseDuration;
						}
					}
					return null;
				case HostPortal:
		...
		function updateClassComponent(current, workInProgress, Component, nextProps, renderLanes) {
			switch (shouldError(workInProgress)) {
				...
				case true:
					...
					workInProgress.lanes = mergeLanes(workInProgress.lanes, lane);
					enqueueCapturedUpdate(workInProgress, createClassErrorUpdate(workInProgress, createCapturedValue(error$1, workInProgress), lane));
			}
			if (workInProgress.type !== workInProgress.elementType) {
				var innerPropTypes = Component.propTypes;
				if (innerPropTypes) checkPropTypes(innerPropTypes, nextProps, "prop", getComponentNameFromType(Component));
		...
		function updateHostRoot(current, workInProgress, renderLanes) {
			...
			if (supportsHydration && prevState.isDehydrated) {
				...
				if (workInProgress.flags & ForceClientRender) return mountHostRootWithoutHydrating(current, workInProgress, nextChildren, renderLanes, /* @__PURE__ */ new Error("There was an error while hydrating. Because the error happened outside of a Suspense boundary, the entire root will switch to client rendering."));
				else if (nextChildren !== prevChildren) return mountHostRootWithoutHydrating(current, workInProgress, nextChildren, renderLanes, /* @__PURE__ */ new Error("This root received an early update, before anything was able hydrate. Switched the entire root to client rendering."));
				else {
				...
				if (prevState !== null) {
					...
					if (_dehydrated !== null) {
						if (!didSuspend) return updateDehydratedSuspenseComponent(current, workInProgress, _dehydrated, prevState, renderLanes);
							...
							workInProgress.flags &= ~ForceClientRender;
							return retrySuspenseComponentWithoutHydrating(current, workInProgress, renderLanes, /* @__PURE__ */ new Error("There was an error while hydrating this Suspense boundary. Switched to client rendering."));
		...
		function updateDehydratedSuspenseComponent(current, workInProgress, suspenseInstance, suspenseState, renderLanes) {
			...
			if (isSuspenseInstanceFallback(suspenseInstance)) return retrySuspenseComponentWithoutHydrating(current, workInProgress, renderLanes, /* @__PURE__ */ new Error("The server could not finish this Suspense boundary, likely due to an error during server rendering. Switched to client rendering."));
			...
			if (didReceiveUpdate || hasContextChanged) {
				...
				if (root !== null) {
					...
					if (attemptHydrationAtLane !== NoLane && attemptHydrationAtLane !== suspenseState.retryLane) {
						...
						scheduleUpdateOnFiber(current, attemptHydrationAtLane, NoTimestamp);
					}
				}
				renderDidSuspendDelayIfPossible();
				return retrySuspenseComponentWithoutHydrating(current, workInProgress, renderLanes, /* @__PURE__ */ new Error("This Suspense boundary received an update before it finished hydrating. This caused the boundary to switch to client rendering. The usual way to fix this is to wrap the original update in startTransition."));
		...
		function beginWork(current, workInProgress, renderLanes) {
			...
			switch (workInProgress.tag) {
				...
				case HostComponent: return updateHostComponent$1(current, workInProgress, renderLanes);
				case HostText: return updateHostText$1(current, workInProgress);
		...
		*/
		...
		function commitLayoutEffectOnFiber(finishedRoot, current, finishedWork, committedLanes) {
			if ((finishedWork.flags & LayoutMask) !== NoFlags) switch (finishedWork.tag) {
				...
				case ScopeComponent:
				case OffscreenComponent:
				case LegacyHiddenComponent: break;
				default: throw new Error("This unit of work tag should not have side-effects. This error is likely caused by a bug in React. Please file an issue.");
			}
			if (!offscreenSubtreeWasHidden) {
		...
		function commitUnmount(finishedRoot, current, nearestMountedAncestor) {
			...
			switch (current.tag) {
				...
				case HostComponent:
					...
					return;
				case HostPortal:
					if (supportsMutation) unmountHostComponents(finishedRoot, current, nearestMountedAncestor);
		...
		function emptyPortalContainer(current) {
			...
			var containerInfo = current.stateNode.containerInfo;
			replaceContainerChildren(containerInfo, createContainerChildSet(containerInfo));
		}
		...
		function commitDeletion(finishedRoot, current, nearestMountedAncestor) {
			if (supportsMutation) unmountHostComponents(finishedRoot, current, nearestMountedAncestor);
			else commitNestedUnmounts(finishedRoot, current, nearestMountedAncestor);
		...
		function commitWork(current, finishedWork) {
			...
			switch (finishedWork.tag) {
				...
				case HostText:
					...
					commitTextUpdate(textInstance, current !== null ? current.memoizedProps : newText, newText);
					return;
				case HostRoot:
					if (supportsHydration) {
						if (current !== null) {
			...
			if (flags & Visibility) switch (finishedWork.tag) {
				case SuspenseComponent:
					if (finishedWork.memoizedState !== null) {
						...
						if (!(_current !== null && _current.memoizedState !== null)) markCommitTimeOfFallback();
					}
					break;
				case OffscreenComponent:
					...
					if (supportsMutation) hideOrUnhideAllChildren(offscreenBoundary, _isHidden);
					if (_isHidden) {
						if (!_wasHidden) {
		...
		function findFiberRootForHostRoot(hostRoot) {
			...
			if (maybeFiber != null) {
				if (typeof maybeFiber.memoizedProps["data-testname"] !== "string") throw new Error("Invalid host root specified. Should be either a React container or a node with a testname attribute.");
				return maybeFiber;
			} else {
				var fiberRoot = findFiberRoot(hostRoot);
				if (fiberRoot === null) throw new Error("Could not find React container within specified host subtree.");
		...
		function matchSelector(fiber, selector) {
			switch (selector.$$typeof) {
				case COMPONENT_TYPE:
					...
					break;
				case HAS_PSEUDO_CLASS_TYPE: return hasMatchingPaths(fiber, selector.value);
				case ROLE_TYPE:
					if (fiber.tag === HostComponent) {
				...
				case TEST_NAME_TYPE:
					...
					break;
				default: throw new Error("Invalid selector type specified.");
			}
			return false;
		}
		...
		function findAllNodes(hostRoot, selectors) {
			if (!supportsTestSelectors) throw new Error("Test selector API is not supported by this renderer.");
			var matchingFibers = findPaths(findFiberRootForHostRoot(hostRoot), selectors);
			var instanceRoots = [];
		...
		function isLegacyActEnvironment(fiber) {
			...
			return warnsIfNotActing && typeof jest !== "undefined" && isReactActEnvironmentGlobal !== false;
		}
		...
		function requestEventTime() {
			if ((executionContext & (RenderContext | CommitContext)) !== NoContext) return now$1();
			...
			if (newCallbackPriority === SyncLane) {
				...
				if (supportsMicrotasks) {
					if (ReactCurrentActQueue$1.current !== null) ReactCurrentActQueue$1.current.push(flushSyncCallbacks);
						...
						if (executionContext === NoContext) flushSyncCallbacks();
					});
				} else scheduleCallback$1(ImmediatePriority, flushSyncCallbacks);
		...
		function pingSuspendedRoot(root, wakeable, pingedLanes) {
			...
			if (workInProgressRoot === root && isSubsetOfLanes(workInProgressRootRenderLanes, pingedLanes)) {
				if (workInProgressRootExitStatus === RootSuspendedWithDelay || workInProgressRootExitStatus === RootSuspended && includesOnlyRetries(workInProgressRootRenderLanes) && now$1() - globalMostRecentFallbackTime < FALLBACK_THROTTLE_MS) prepareFreshStack(root, NoLanes);
		...
		beginWork$1 = function(current, unitOfWork, lanes) {
			...
			try {
				...
				if (originalError !== null && typeof originalError === "object" && typeof originalError.then === "function") throw originalError;
				resetContextDependencies();
				resetHooksAfterThrow();
		...
		exports$1.attemptContinuousHydration = attemptContinuousHydration;
		exports$1.attemptHydrationAtCurrentPriority = attemptHydrationAtCurrentPriority;
		exports$1.attemptSynchronousHydration = attemptSynchronousHydration;
		exports$1.batchedUpdates = batchedUpdates;
		exports$1.createComponentSelector = createComponentSelector;
		exports$1.createContainer = createContainer;
		exports$1.createHasPseudoClassSelector = createHasPseudoClassSelector;
		exports$1.createHydrationContainer = createHydrationContainer;
		exports$1.createPortal = createPortal;
		exports$1.createRoleSelector = createRoleSelector;
		exports$1.createTestNameSelector = createTestNameSelector;
		exports$1.createTextSelector = createTextSelector;
		exports$1.deferredUpdates = deferredUpdates;
		exports$1.discreteUpdates = discreteUpdates;
		exports$1.findAllNodes = findAllNodes;
		exports$1.findBoundingRects = findBoundingRects;
		exports$1.findHostInstance = findHostInstance;
		exports$1.findHostInstanceWithNoPortals = findHostInstanceWithNoPortals;
		exports$1.findHostInstanceWithWarning = findHostInstanceWithWarning;
		exports$1.flushControlled = flushControlled;
		exports$1.flushPassiveEffects = flushPassiveEffects;
		exports$1.flushSync = flushSync;
		exports$1.focusWithin = focusWithin;
		exports$1.getCurrentUpdatePriority = getCurrentUpdatePriority;
		exports$1.getFindAllNodesFailureDescription = getFindAllNodesFailureDescription;
		exports$1.getPublicRootInstance = getPublicRootInstance;
		exports$1.injectIntoDevTools = injectIntoDevTools;
		exports$1.isAlreadyRendering = isAlreadyRendering;
		exports$1.observeVisibleRects = observeVisibleRects;
		exports$1.registerMutableSourceForHydration = registerMutableSourceForHydration;
		exports$1.runWithPriority = runWithPriority;
		exports$1.shouldError = shouldError;
		exports$1.shouldSuspend = shouldSuspend;
		exports$1.updateContainer = updateContainer;
		return exports$1;
	};
}));
//#endregion
...
//#endregion
//#region node_modules/@react-three/fiber/dist/events-776716bd.esm.js
...
function createRenderer(_roots, _getEventPriority) {
	...
	function removeChild(parentInstance, child, dispose) {
		if (child) {
			...
			const shouldDispose = !isPrimitive && (dispose === void 0 ? child.dispose !== null : dispose);
			if (!isPrimitive) {
				var _child$__r3f6;
			...
			if (shouldDispose && child.dispose && child.type !== "Scene") {
				...
				if (typeof IS_REACT_ACT_ENVIRONMENT === "undefined") (0, import_scheduler.unstable_scheduleCallback)(import_scheduler.unstable_IdlePriority, callback);
				else callback();
			}
	...
	function switchInstance(instance, type, newProps, fiber) {
		...
		if (newInstance.raycast && newInstance.__r3f.eventCount) findInitialRoot(newInstance).getState().internal.interaction.push(newInstance);
	...
	return {
		reconciler: (0, import_react_reconciler.default)({
			createInstance,
			removeChild,
			appendChild,
			...
			prepareUpdate(instance, _type, oldProps, newProps) {
				...
				if (((_instance$__r3f3 = instance == null ? void 0 : instance.__r3f) != null ? _instance$__r3f3 : {}).primitive && newProps.object && newProps.object !== instance) return [true];
				else {
...
*/
var useIsomorphicLayoutEffect$1 = typeof window !== "undefined" && ((_window$document = window.document) != null && _window$document.createElement || ((_window$navigator = window.navigator) == null ? void 0 : _window$navigator.product) === "ReactNative") ? import_react.useLayoutEffect : import_react.useEffect;
function useMutableCallback(fn) {
	const ref = import_react.useRef(fn);
	useIsomorphicLayoutEffect$1(() => void (ref.current = fn), [fn]);
	return ref;
}
function Block({ set }) {
...
function attach(parent, child, type) {
	if (is.str(type)) {
		if (INDEX_REGEX.test(type)) {
			const { target, key } = resolve(parent, type.replace(INDEX_REGEX, ""));
...
function applyProps$1(instance, data) {
	...
	for (let i = 0; i < changes.length; i++) {
		...
		if (isEvent && localState) {
			...
			} else if (targetProp.copy && value && value.constructor && (__DEV__ ? targetProp.constructor.name === value.constructor.name : targetProp.constructor === value.constructor)) targetProp.copy(value);
			...
			if ((_currentInstance$key = currentInstance[key]) != null && _currentInstance$key.isTexture && currentInstance[key].format === 1023 && currentInstance[key].type === 1009 && rootState) {
	...
	if (!(changes.length === 1 && changes[0][0] === "onUpdate") && changes.length && (_instance$__r3f2 = instance.__r3f) != null && _instance$__r3f2.parent) updateInstance(instance);
	return instance;
}
...
function getEventPriority() {
	...
	switch ((_globalScope$event = globalScope.event) == null ? void 0 : _globalScope$event.type) {
		case "click":
		case "contextmenu":
		case "dblclick":
		case "pointercancel":
		case "pointerdown":
		case "pointerup": return import_constants.DiscreteEventPriority;
		case "pointermove":
		case "pointerout":
		case "pointerover":
		case "pointerenter":
		case "pointerleave":
		case "wheel": return import_constants.ContinuousEventPriority;
		default: return import_constants.DefaultEventPriority;
	}
}
/**
	...
	function handleIntersects(intersections, event, delta, callback) {
		...
		if (intersections.length) {
			...
			for (const hit of intersections) {
				const { raycaster, pointer, camera, internal } = getRootState(hit.object) || rootState;
...
function useInstanceHandle(ref) {
	...
	useIsomorphicLayoutEffect$1(() => void (instance.current = ref.current.__r3f), [ref]);
	return instance;
}
function useStore() {
...
function useFrame(callback, renderPriority = 0) {
	...
	useIsomorphicLayoutEffect$1(() => subscribe(ref, renderPriority, store), [
		renderPriority,
		subscribe,
		store
	]);
...
useLoader.clear = function(Proto, input) {
	return clear([Proto, ...Array.isArray(input) ? input : [input]]);
};
var roots = /* @__PURE__ */ new Map();
var { invalidate, advance } = createLoop(roots);
var { reconciler, applyProps } = createRenderer(roots, getEventPriority);
var shallowLoose = {
	objects: "shallow",
	strict: false
};
...
function createRoot(canvas) {
	...
	return {
		configure(props = {}) {
			...
			if (!configured) applyProps(gl, {
				outputEncoding: linear ? 3e3 : 3001,
				toneMapping: flat ? 0 : 4
			});
...
function Portal({ state = {}, children, container }) {
	...
	*    {createPortal(...)} */
	...
	const inject = import_react.useCallback((rootState, injectState) => {
		const intersect = { ...rootState };
		Object.keys(rootState).forEach((key) => {
			if (privateKeys.includes(key) || rootState[key] !== injectState[key] && injectState[key]) delete intersect[key];
		});
		let viewport = void 0;
		if (injectState && size) {
	...
	const [usePortalStore] = import_react.useState(() => {
...
var act = import_react.unstable_act;
var DOM_EVENTS = {
	onClick: ["click", false],
...
var CanvasImpl = /*#__PURE__*/ import_react.forwardRef(function Canvas({ children, fallback, resize, style, gl, events = createPointerEvents, eventSource, eventPrefix, shadows, linear, flat, legacy, orthographic, frameloop, dpr, performance, raycaster, camera, scene, onPointerMissed, onCreated, ...props }, forwardedRef) {
	...
	import_react.useImperativeHandle(forwardedRef, () => canvasRef.current);
...
*/
var Canvas = /*#__PURE__*/ import_react.forwardRef(function CanvasWrapper(props, ref) {
	return /*#__PURE__*/ (0, import_jsx_runtime.jsx)(FiberProvider, { children: /*#__PURE__*/ (0, import_jsx_runtime.jsx)(CanvasImpl, {
		...props,
		ref
	}) });
});
//#endregion
export { useThree as A, threeTypes as C, useInstanceHandle as D, useGraph as E, preload as M, suspend as N, useLoader as O, roots as S, useFrame as T, flushSync as _, addTail as a, reconciler as b, buildGraph as c, createPointerEvents as d, createPortal as f, flushGlobalEffects as g, extend as h, addEffect as i, clear as j, useStore as k, context as l, dispose as m, act as n, advance as o, createRoot as p, addAfterEffect as r, applyProps as s, Canvas as t, createEvents as u, getRootState as v, unmountComponentAtNode as w, render as x, invalidate as y };
