import { BufferGeometry, Color, FileLoader, Float32BufferAttribute, Loader } from "./three.js";
//#region node_modules/three/examples/jsm/loaders/PLYLoader.js
/**
	...
	parse(data) {
			...
			for (let i = 0; i < lines.length; i++) {
				...
				switch (lineType) {
					...
					case "comment":
						...
						break;
					case "element":
						if (currentElement !== void 0) header.elements.push(currentElement);
						currentElement = {};
		...
		function parseASCIINumber(n, type) {
			switch (type) {
				...
				case "uint16":
				case "int32":
				case "uint32": return parseInt(n);
				case "float":
				case "double":
				case "float32":
				case "float64": return parseFloat(n);
			}
		}
		...
		function postProcess(buffer) {
			...
			for (const customProperty of Object.keys(scope.customPropertyMapping)) if (buffer[customProperty].length > 0) geometry.setAttribute(customProperty, new Float32BufferAttribute(buffer[customProperty], scope.customPropertyMapping[customProperty].length));
		...
		if (data instanceof ArrayBuffer) {
			...
			if (header.format === "ascii") geometry = parseASCII(new TextDecoder().decode(bytes), header);
